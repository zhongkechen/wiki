...
assert(qmd_files.length == 370, "Expected 370 QMD pages, found #{qmd_files.length}")
assert(asset_files.length == 171, "Expected 171 attachments, found #{asset_files.length}")
assert(
  MOIN_ADDITIONAL_PAGE_NAMES.length == 115,
  "Expected 115 additional source pages"
)
assert(
...
Dir.mktmpdir("check-other-pages-migration") do |temporary_root|
  ...
  assert(stdout.include?("Converted 370 pages"), "Unexpected page count:\n#{stdout}")
  assert(
