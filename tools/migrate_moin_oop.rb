...
SOURCE_SNAPSHOT_PATH = File.expand_path("data/moin_oop_sources.json", __dir__)
SOURCE_SNAPSHOT =
...
def prepare_course_source(source)
  ...
  source.lines.map do |line|
    replacement = headings[line.strip]
...
Dir.mktmpdir("migrate-moin-oop") do |temporary_root|
  ...
  course_body = convert_moin(
    prepare_course_source(COURSE_SOURCE),
    owner: COURSE_NAME,
    context: :course
  )
  course_page = [
